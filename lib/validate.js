...
var validate = function(file, callback) {
  ...
  if (file.istmpl) {
    ...
    // ADDED BY agscheidle: this should help ignore attributes that contain bindings...
    //{ U+007B LEFT CURLY BRACKET, decimal: 123, HTML: &#123;, UTF-8: 0x7B, block: Basic Latin
    //} U+007D RIGHT CURLY BRACKET, decimal: 125, HTML: &#125;, UTF-8: 0x7D, block: Basic Latin
    //.replace(/\u201d/g, '&#8221;');
    content = content.replace('u007b','');
    content = content.replace('&#123;','');
    content = content.replace('{','');
    content = content.replace('u007d','');
    content = content.replace('&#125;','');
    content = content.replace('}','');
